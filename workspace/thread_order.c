...
void	print(t_thread *th, char *s)
{
	pthread_mutex_lock(&th->share->print_mutex);
	printf("%lld %d %s\n", get_elapsed_time(th->share), th->id, s);
	pthread_mutex_unlock(&th->share->print_mutex);
}
...
int	main(void)
{
	...
	for (int i=0;i<share.nb_of_th;i++)
		pthread_create(&thread[i].th, NULL, &routine, (void *)&thread[i]);
	for (int i=0;i<share.nb_of_th;i++)
